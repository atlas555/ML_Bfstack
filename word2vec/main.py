...
class word2vec():
    ...
    def train_by_sentence(self, input_sentence=[]):
        ...
        if self.train_sents_num % 100 == 0:
            ...
            print("{a} sentences dealed, loss: {b} , learning_rate={c}".format(a=self.train_sents_num, b=self.train_loss_k10, c=self.learning_rate))
            if loss_val >= np.max(self.train_loss_records):
...
if __name__ == '__main__':
    ...
    raw_word_list = []
    sentence_list = []
    with io.open('111.txt', 'r', encoding='gbk') as f:
        line = f.readline()
        while line:
            while '\n' in line:
                line = line.replace('\n', '')
