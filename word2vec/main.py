...
class word2vec():
    def __init__(self,
        ...
        if model_path is not None:
            ...
            self.vocal_size = vocab_list.__len__()
            ...
            for i in range(self.vocal_size):
                self.word2id[self.vocal_size[i]] = i
            ...
            self.train_loss_record = collections.deque(maxlen=10)
    ...
    def init_op(self):
        ...
        self.sumary_writer = tf.train_SummaryWriter(self.log_dir,self.graph)
    ...
    def build_graph(self):
        ...
        with self.graph.as_default():
            ...
            self.embedding_dict = tf.Variable(tf.random_uniform([self.vocal_size,self.embedding_size],-1.0,1.0))
            self.nce_weight = tf.Variable(tf.truncated_normal([self.vocal_size,self.embedding_size],stddev=1.0/math.sqrt(self.embedding_size)))
            self.nce_biases = tf.Variable(tf.zeros(self.vocal_size))

            # 将输入序列向量化
            embed = tf.nn.embedding_lookup(self.embedding_dict,self.train_inputs)

            # 得到MCE的损失
            self.loss = tf.reduce_mean(tf.nn.nce_loss(weight=self.nce_weight,biases = self.nce_biases,labels = self.train_labels,num_sampled = self.num_sampled,num_classes = self.vocal_size))
    ...
    def train_by_sentence(self,input_sentence=[]):
        ...
        batch_labels = np.reshape(batch_labels,[batch_labels.__len__()])

        feed_dict = {self.train_inputs:batch_inputs,
                     self.train_labels:batch_labels}

        _,loss_val, summary_str = self.sess.run([self.train_op,self.loss,self.merged_summary_op],feed_dict)
...
if __name__=='__main__':
    ...
    # step 3 创建模型，训练
    w2c = word2vec(vocab_list=word_list,# 词典集
                   embedding_size=200,
                   win_len=2,
                   learning_rate=1,
